...
L'obiettivo di questo progetto è duplice. Da un lato, si vuole comprendere \emph{quali} fattori organizzativi, demografici ed economici sono più associati alla decisione di un dipendente di lasciare l'azienda, attraverso un'analisi esplorativa dei dati (Sezioni~\ref{sec:metodologia} e \ref{sec:risultati}). Dall'altro, si vuole costruire un modello predittivo in grado di stimare la probabilità di abbandono di un dipendente, confrontando quattro approcci di classificazione supervisionata con caratteristiche diverse: un modello lineare e interpretabile (regressione logistica), un modello basato su distanza (k-NN), un modello ad albero in ensemble (Random Forest), e un modello che usa una strttura ad albero di decisione (Decision Tree).
...
\begin{itemize}
    ...
    \item \textbf{Decision Tree}, modello di classificazione basato su una struttura gerarchica di regole decisionali del tipo \emph{if--then}. 
      L’albero suddivide ricorsivamente lo spazio delle feature scegliendo, a ogni nodo, la variabile e la soglia che massimizzano la purezza delle classi (criterio di Gini). 
      Il risultato è un modello interpretabile, che permette di visualizzare esplicitamente il percorso decisionale che conduce alla previsione di \texttt{Attrition = Yes}.
...
La regressione logistica non produce direttamente una classe, ma una\emph{probabilità} che l'osservazione appartenga alla classe positiva. Dato un vettore di feature $\mathbf{x}$, il modello calcola:
...
dove $\mathbf{w}$ è il vettore dei coefficienti, $b$ è l'intercetta e 
$\sigma(z)$ è la funzione sigmoide:

\begin{equation}
\sigma(z) = \frac{1}{1 + e^{-z}}.
\end{equation}

La sigmoide mappa qualsiasi valore reale in un intervallo compreso tra $0$ e $1$, rendendola adatta a rappresentare probabilità. 
Per ottenere una decisione finale, la probabilità viene confrontata con una soglia (tipicamente $0.5$):

\begin{equation}
\hat{y} =
\begin{cases}
1 & \text{se } P(y=1 \mid \mathbf{x}) \ge 0.5, \\
0 & \text{altrimenti}.
\end{cases}
\end{equation}

In questo modo la regressione logistica separa chiaramente il processo 
di \emph{stima della probabilità} dal processo di \emph{classificazione}.
...
La Tabella~\ref{tab:metriche} riassume le metriche principali calcolate sul test set (441 osservazioni) per i quattro modelli, con riferimento alla classe minoritaria \texttt{Attrition = Yes}.
...
Il k-NN, in particolare, risulta penalizzato dalla dimensionalità elevata del problema dopo il one-hot encoding (oltre 40 feature), un fenomeno noto come \emph{curse of dimensionality}, che rende la nozione di ``vicinanza'' euclidea meno significativa. Inoltre, il Decision Tree, nonostante la sua alta interpretabilità, non si avvicina alle prestazione degli altri tre modelli.
